...
while true; do
    ...
    if validate_ip "$ZABBIX_IP"; then break; else warning "Invalid IP. Please enter a valid IPv4 address."; fi
done
...
{
    wget -q https://repo.zabbix.com/zabbix/7.4/${OS}/$(lsb_release -cs)/amd64/zabbix-release_7.4-1+${OS}$(lsb_release -rs)_all.deb
    sudo dpkg -i zabbix-release_7.4-1+${OS}$(lsb_release -rs)_all.deb >/dev/null
...
{
    sudo $PM install -y zabbix-server-mysql zabbix-frontend-php zabbix-apache-conf \
    zabbix-sql-scripts zabbix-agent >/dev/null
} & spinner
success "Zabbix server, frontend, agent installed"
...
info "Enabling and starting Zabbix and Apache services..."
{
    sudo systemctl enable --now zabbix-server zabbix-agent apache2
} & spinner
success "Services started"

info "Setting Zabbix Admin password in database..."
{
